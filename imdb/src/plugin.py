...
try:
    ...
    unichr = chr  # In Python 3, mappa 'unichr' su 'chr'
    ...
    def IMDBquery(self, response):
        ...
        if start != -1:
            ...
            if re.search("<title>Find - IMDb</title>", html):
                ...
                searchresultmask = re.compile(r'<tr class="findResult (?:odd|even)">.*?<td class="result_text"> (<a href="/title/(tt\d{7,7})/.*?"\s?>(.*?)</a>.*?)</td>', re.DOTALL)
